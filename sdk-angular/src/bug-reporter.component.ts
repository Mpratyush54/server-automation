...
@Component({
  selector: 'platform-bug-reporter',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <button
      class="platform-bug-btn"
      (click)="toggle()"
      title="Report a bug"
    >🐛</button>

    <div class="platform-bug-drawer" *ngIf="isOpen">
      <div class="platform-bug-header">Report a Bug</div>
      <div *ngIf="sent" class="platform-bug-sent">
        Bug report submitted. Thank you!
      </div>
      <ng-container *ngIf="!sent">
        <div class="platform-bug-body">
          <textarea
            [(ngModel)]="description"
            placeholder="Describe the bug you encountered..."
            class="platform-bug-textarea"
          ></textarea>
          <div class="platform-bug-hint">
            Console logs and network history will be attached automatically.
          </div>
        </div>
        <div class="platform-bug-footer">
          <button class="platform-bug-cancel" (click)="toggle()">Cancel</button>
          <button
            class="platform-bug-submit"
  ...
  styles: [`
    ...
    .platform-bug-header { padding: 16px; border-bottom: 1px solid #333; font-weight: 600; font-size: 14px; }
    .platform-bug-sent { padding: 40px 16px; text-align: center; color: #22c55e; }
    .platform-bug-body { padding: 16px; flex: 1; overflow: auto; }
    .platform-bug-textarea {
    ...
    .platform-bug-hint { margin-top: 8px; font-size: 11px; color: #888; }
    .platform-bug-footer { padding: 12px 16px; border-top: 1px solid #333; display: flex; gap: 8px; }
    .platform-bug-cancel {
    ...
    .platform-bug-submit:disabled { opacity: 0.6; cursor: not-allowed; }
  `]
})
