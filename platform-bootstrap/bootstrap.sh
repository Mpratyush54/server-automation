...
#
#  Usage:
#    curl -fsSL https://raw.githubusercontent.com/Mpratyush54/SERVER-automation/main/platform-bootstrap/bootstrap.sh | bash
#    — or —
...
retry() {
  ...
  until "$@"; do
    ...
    if [ $count -lt $retries ]; then
      warn "Retry $count/$retries for: $*"
      sleep 5
    else
      error "Command failed after $retries retries: $*"
      return $exit
    fi
  done
  return 0
...
install_ingress() {
  ...
  if helm list -n ingress-nginx 2>/dev/null | grep -q ingress-nginx; then
    ...
    retry 3 helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx \
...
install_certmanager() {
  ...
  retry 3 helm upgrade --install cert-manager cert-manager/cert-manager \
...
install_databases() {
  ...
  if ! helm list -n databases 2>/dev/null | grep -q postgresql; then
    ...
    retry 3 helm upgrade --install postgresql bitnami/postgresql \
      --namespace databases \
  ...
  if ! helm list -n databases 2>/dev/null | grep -q mongodb; then
    ...
    retry 3 helm upgrade --install mongodb bitnami/mongodb \
      --namespace databases \
  ...
  if ! helm list -n databases 2>/dev/null | grep -q redis; then
    ...
    retry 3 helm upgrade --install redis bitnami/redis \
      --namespace databases \
...
install_minio() {
  ...
  retry 3 helm upgrade --install minio bitnami/minio \
    --namespace storage \
...
install_monitoring() {
  ...
  retry 3 helm upgrade --install kube-prometheus prometheus-community/kube-prometheus-stack \
  ...
  if ! helm list -n monitoring 2>/dev/null | grep -q loki; then
    ...
    retry 3 helm upgrade --install loki grafana/loki-stack \
      --namespace monitoring \
...
install_oauth2_proxy() {
  ...
  retry 3 helm upgrade --install oauth2-proxy oauth2-proxy/oauth2-proxy \
...
install_portainer() {
  ...
  retry 3 helm upgrade --install portainer portainer/portainer \
    --namespace portainer \
