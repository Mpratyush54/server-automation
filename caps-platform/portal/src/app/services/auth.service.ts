...
export class AuthService {
  ...
  login(email: string): Observable<any> {
    return this.http.post<any>(`${this.base}/auth/login`, { email }).pipe(
      tap(res => {
        if (res.token) {
          localStorage.setItem('caps_auth_token', res.token);
  ...
  logout() {
    localStorage.removeItem('caps_auth_token');
    localStorage.removeItem('caps_user_profile');
    this.router.navigate(['/login']);
  }

  getToken(): string | null {
    return localStorage.getItem('caps_auth_token');
  }

  getUser(): any {
    const profile = localStorage.getItem('caps_user_profile');
