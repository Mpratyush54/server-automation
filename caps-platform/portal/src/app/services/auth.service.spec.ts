...
describe('AuthService', () => {
  ...
  describe('getToken', () => {
    ...
    it('should return stored token', () => {
      localStorage.setItem('caps_auth_token', 'test-token');
  ...
  describe('getUser', () => {
    ...
    it('should return parsed user profile', () => {
      ...
      localStorage.setItem('caps_user_profile', JSON.stringify(user));
  ...
  describe('getRole', () => {
    ...
    it('should return user role from profile', () => {
      localStorage.setItem('caps_user_profile', JSON.stringify({ role: 'devops' }));
  ...
  describe('role checks', () => {
    ...
    it('isDevOps should return false for developer role', () => {
      localStorage.setItem('caps_user_profile', JSON.stringify({ role: 'developer' }));
      expect(service.isDevOps()).toBeFalse();
    });

    it('isTechLead should return true for tech_lead role', () => {
      localStorage.setItem('caps_user_profile', JSON.stringify({ role: 'tech_lead' }));
  ...
  describe('logout', () => {
    it('should clear localStorage', () => {
      localStorage.setItem('caps_auth_token', 'token');
      localStorage.setItem('caps_user_profile', '{}');

      service.logout();

      expect(localStorage.getItem('caps_auth_token')).toBeNull();
      expect(localStorage.getItem('caps_user_profile')).toBeNull();
    });
  });
});
