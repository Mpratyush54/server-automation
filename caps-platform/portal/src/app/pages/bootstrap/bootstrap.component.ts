...
<div>
  ...
  <div class="card" style="background:linear-gradient(135deg,rgba(99,102,241,0.08),rgba(139,92,246,0.08));border-color:rgba(99,102,241,0.25);margin-bottom:24px;">
    ...
    <p style="margin:0;color:var(--color-text-secondary);font-size:0.9rem;">Get a fresh Ubuntu server running CAPS Platform end-to-end with a single command.</p>
  </div>
  ...
  <div *ngIf="tab === 'start'">
    ...
    <div class="card" style="padding:24px;background:rgba(0,0,0,0.35);border-color:rgba(99,102,241,0.3);margin-bottom:20px;">
      ...
      <div style="position:relative;">
        <pre style="background:#0b0f19;padding:18px 20px;border-radius:10px;font-size:0.84rem;color:#34d399;margin:0;overflow-x:auto;border:1px solid rgba(52,211,153,0.15);">curl -fsSL https://raw.githubusercontent.com/your-org/caps/main/caps-bootstrap/bootstrap.sh -o bootstrap.sh
...
✔ GitHub configured (webhook: https://caps.mycompany.com/api/webhooks/github)
...
✔ SMTP configured</pre>
  ...
  <div *ngIf="tab === 'script'">
    ...
    <p style="color:var(--color-text-secondary);font-size:0.82rem;margin:0 0 16px 0;">
      The script is fully self-contained. It tracks completed steps in <code style="background:rgba(255,255,255,0.05);padding:1px 5px;border-radius:3px;">/etc/caps/.bootstrap_state</code>
    ...
    <div class="card" style="padding:20px;margin-bottom:16px;background:rgba(15,23,42,0.6);">
      ...
      <div style="display:grid;grid-template-columns:auto 1fr;gap:6px 20px;font-size:0.8rem;">
        ...
        <code style="color:#a78bfa;">CAPS_DOMAIN=caps.co.com</code><span style="color:var(--color-text-secondary);">Pre-fill domain (skips prompt)</span>
        <code style="color:#a78bfa;">CAPS_IMAGE_TAG=v2.1.0</code><span style="color:var(--color-text-secondary);">Specific image version to deploy</span>
        <code style="color:#a78bfa;">CAPS_REPO_URL=ghcr.io/…</code><span style="color:var(--color-text-secondary);">Container registry URL</span>
      </div>
    </div>
...
# CAPS Platform — Full Server Bootstrap v2.0.0
# Run: sudo ./bootstrap.sh
# Docs: https://github.com/your-org/caps/blob/main/caps-bootstrap/README.md
#
...
# Portainer, Infisical, and finally CAPS Platform itself.
# It also walks you through configuring GitHub, GitLab, ClickUp, and SMTP.
#
# Re-run safely — completed steps are tracked in /etc/caps/.bootstrap_state

curl -fsSL https://raw.githubusercontent.com/your-org/caps/main/caps-bootstrap/bootstrap.sh | sudo bash`;

  installCmd = `curl -fsSL https://raw.githubusercontent.com/your-org/caps/main/caps-bootstrap/bootstrap.sh -o bootstrap.sh
chmod +x bootstrap.sh
sudo ./bootstrap.sh`;
  ...
  phases = [
    ...
    { phase: 6,  icon: '🗂️', name: 'Namespaces',           desc: 'caps-platform, databases, monitoring, storage, argocd...' },
    ...
    { phase: 15, icon: '🚀', name: 'CAPS Platform',       desc: 'API + Portal deployed with all env vars pre-wired from config' },
  ...
  async loadStatus() {
    ...
    try {
      ...
      // Map to display list
      this.statusServices = [
        { label: 'CAPS API',      namespace: 'caps-platform', status: services['caps-api']      || 'unknown', detail: 'Backend REST API' },
        { label: 'CAPS Portal',   namespace: 'caps-platform', status: services['caps-portal']   || 'unknown', detail: 'Angular Frontend' },
