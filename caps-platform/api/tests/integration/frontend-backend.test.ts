...
const mockUserRepository = {
  findOne: jest.fn().mockImplementation(({ where }) => {
    if (where.email === 'devops@caps.io' || where.id === mockUser.id) {
      return Promise.resolve(mockUser);
    }
    if (where.email === 'dev@caps.io' || where.id === '00000002') {
...
jest.mock('../../src/lib/database-service', () => ({
  provisionPostgresDb: jest.fn().mockResolvedValue({
    dbName: 'caps_testproject_development',
    username: 'caps_testproject_development_user',
...
describe('Frontend & Backend Integration Tests', () => {
  ...
  describe('POST /api/auth/login', () => {
    it('should authenticate a valid user and return a JWT token with user object', async () => {
      const res = await request(app)
        .post('/api/auth/login')
        .send({ email: 'devops@caps.io' });

      expect(res.status).toBe(200);
      expect(res.body.token).toBeDefined();
      expect(res.body.user.email).toBe('devops@caps.io');
  ...
  describe('POST /api/projects/:projectId/databases/provision', () => {
    it('should provision a Postgres database for DevOps user', async () => {
      ...
      expect(res.body.dbName).toBe('caps_testproject_development');
      expect(res.body.username).toBe('caps_testproject_development_user');
    ...
    it('should fail with 403 Forbidden for a regular developer user', async () => {
      const devToken = jwt.sign(
        { id: '00000002', email: 'dev@caps.io', name: 'Dev', role: 'developer' },
        JWT_SECRET,
        { expiresIn: '24h' }
      );
