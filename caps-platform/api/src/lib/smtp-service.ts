...
export async function sendMail(
  ...
  await transport.sendMail({
    from: `"${opts.fromName || 'CAPS Platform'}" <${opts.fromEmail}>`,
...
export async function testSmtpConnection(opts: SmtpOptions, testTo: string): Promise<{ success: boolean; error?: string }> {
  try {
    await sendMail(opts, testTo, 'CAPS Platform — SMTP Test', `
      <div style="font-family:sans-serif;max-width:600px;margin:40px auto;">
        <h2 style="color:#7c3aed;">✅ SMTP Configuration Working</h2>
        <p>Your SMTP configuration has been verified successfully. CAPS Platform will use this to send deployment alerts and notifications.</p>
        <hr style="border:1px solid #e2e8f0;"/>
        <p style="color:#64748b;font-size:12px;">CAPS Platform — Internal Developer Platform</p>
      </div>
    `);
    return { success: true };
  } catch (err: any) {
